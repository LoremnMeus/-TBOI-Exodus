...
local ui = require("EdenAndNether_Extra_scripts.auxiliary.ui")
local funct = {
	f = nil
}		--在这里放置有关于图形的函数。
...
if funct.f == nil then 
	...
	path = string.gsub(path, ":/", ":\\")
	---
	funct.f:Load(path .. "mods/EdenAndNether/resources/font/eid9/eid9_9px.fnt")
	if not funct.f:IsLoaded() then
		funct.f:Load(path .. "mods/EdenAndNether/resources/font/eid9/eid9_9px.fnt")
	end
	if not funct.f:IsLoaded() then
		funct.f:Load("../mods/EdenAndNether/resources/font/eid9/eid9_9px.fnt")
	end
	if not funct.f:IsLoaded() then
		funct.f:Load("../mods/EdenAndNether/resources/font/eid9/eid9_9px.fnt")
	end
