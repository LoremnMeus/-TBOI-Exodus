...
function funct.check_screen_size(v)
	local screensize = funct.GetScreenSize()
...
function funct.copy_sprite(s,s2,params)
	params = params or {}
	s2 = s2 or Sprite()
	if (params.filename or s:GetFilename()) ~= s2:GetFilename() then s2:Load(params.filename or s:GetFilename(),true) end
	if REPENTOGON and params.LoadSheet then
		local ls = s:GetAllLayers()
		for u,v in pairs(ls) do
			local vpath = v:GetSpritesheetPath()
			s2:ReplaceSpritesheet(u,vpath)
		end
		s2:LoadGraphics()
	end
...
function auxi.add_function(f1,f2,deal)
	local f = function(...)
		local ret = f1(...)
		if deal then ret = auxi.check_if_any(deal,ret,f2(...))
		else ret = f2(...) or ret end 
		return ret
	end
	return f
end
...
function funct.SetPoopSpriteState(grid, sprite)
