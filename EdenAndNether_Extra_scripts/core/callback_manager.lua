...
function manager.Init(mod)		--可以添加callback的都塞在这里。
	modReference = mod
	table.insert(manager.items,#manager.items + 1,require("EdenAndNether_Extra_scripts.callbacks.collectible_holder"))
	table.insert(manager.items,#manager.items + 1,require("EdenAndNether_Extra_scripts.callbacks.teleport_holder"))
	table.insert(manager.items,#manager.items + 1,require("EdenAndNether_Extra_scripts.callbacks.next_level_holder"))
	table.insert(manager.items,#manager.items + 1,require("EdenAndNether_Extra_scripts.callbacks.every_entity_holder"))
end
